...
class BoundLinear(Linear):
    ...
    def bound_backward(self, last_uA, last_lA):
        ...
        print('in BoundLinear uA ', uA.shape)
        if len(uA.shape)<3:
            return uA.unsqueeze(0), ubias, lA.unsqueeze(0), lbias
        else:
...
class BoundConv2d(Conv2d):
    ...
    def interval_propagate(self, norm, h_U, h_L, eps):
        ...
        if norm == np.inf:
            ...
            #add here
            ##################
            self.output_shape = deviation.size()[1:]
            self.input_shape = h_U.size()[1:]
            ########################
        else:
            # L2 norm
            mid = h_U
    ...
    def backward_range(self, norm=np.inf, x_U=None, x_L=None, eps=None, C=None, upper=False, lower=True, modules=None):
       ...
       #     h_U=module(h_U)
        ...
        for i, module in enumerate(reversed(modules)):
            #The first BoundLinear layer will will backward
            print(i,module)
            #module(upper_A)
        ...
        print('out upper_A, upper_sum_b',upper_A, upper_sum_b)
        print('out lower_A, lower_sum_b',lower_A, lower_sum_b)
        if ub is None:
            ub = x_U.new([np.inf])
        if lb is None:
            lb = x_L.new([-np.inf]) 
